...
> 本仓库保留下面的 H5/clean-GT 训练网络；针对当前 SigmaStar 三路 RAW 数据（源 noisy、2DNR denoised、3DNR fused）新增的可直接训练版本见 [README_2DPRIOR.md](README_2DPRIOR.md)。该版本以 2DNR 为基线，3DNR 仅在静态平坦且两者一致的位置提供小幅修正，不要求 clean GT。
...
当前 2DNR-prior RAW 融合入口位于 `mamba_2d_prior/`，并默认采用局部全分辨率 + 低分辨率全局的 4 路分组扫描；本 README 前半部分描述的旧 H5/w4 网络仍保留 8 路兼容配置。

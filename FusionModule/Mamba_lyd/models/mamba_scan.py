...
try:  # Official Mamba-2/SSD implementation; optional on CPU-only machines.
    from mamba_ssm import Mamba2 as _OfficialMamba2
except Exception:  # pragma: no cover - CUDA/Triton extension is optional
    _OfficialMamba2 = None
...
def selective_scan_mamba_ssm(
    ...
    # The fused kernel requires u and delta to share a dtype.  Under CUDA AMP,
    # the parameter path can remain float32 while the sequence path is reduced
    # precision, so align the per-token tensors explicitly at this boundary.
    kernel_dtype = input_sequence.dtype
    u = input_sequence.transpose(1, 2).contiguous()
    dt = delta.to(dtype=kernel_dtype).transpose(1, 2).contiguous()
    b = input_matrix.to(dtype=kernel_dtype).transpose(1, 2).contiguous()
    c = readout_matrix.to(dtype=kernel_dtype).transpose(1, 2).contiguous()
    # The Mamba selective-scan CUDA implementation keeps state parameters in
    # float32 even when activations use AMP precision.
    a = state_transition.to(device=input_sequence.device, dtype=torch.float32)
    d = skip.to(device=input_sequence.device, dtype=torch.float32)
...
class Mamba2SSD1D(nn.Module):
    """Thin adapter around the official CUDA Mamba-2 SSD mixer.

    The project deliberately has no slow Python fallback for this path: the
    point of Mamba-2 here is its chunked CUDA SSD kernel on long scan paths.
    ``MambaSelectiveScan1D`` remains the portable Mamba-1 implementation.
    """
    ...
    def __init__(
        ...
        super().__init__()
        if _OfficialMamba2 is None:
            raise RuntimeError(
                "mamba_variant='mamba2' requires the official mamba-ssm package "
                "with Mamba2 support on a CUDA Linux environment"
            )
        if channels <= 0 or state_dim <= 0 or expand <= 0 or conv_kernel <= 0 or groups <= 0 or chunk_size <= 0:
            raise ValueError("Mamba-2 dimensions, groups and chunk_size must be positive")
        effective_head_dim = channels if head_dim is None else head_dim
        inner_channels = channels * expand
        if effective_head_dim <= 0 or inner_channels % effective_head_dim:
            raise ValueError(
                f"Mamba-2 requires expand*channels ({inner_channels}) to be divisible by "
                f"head_dim ({effective_head_dim})"
            )
        self.channels = channels
        self.mixer = _OfficialMamba2(
            d_model=channels,
            d_state=state_dim,
            d_conv=conv_kernel,
            expand=expand,
            headdim=effective_head_dim,
            ngroups=groups,
    ...
    def forward(self, sequence: Tensor) -> Tensor:
        ...
        if not sequence.is_cuda:
            raise RuntimeError("mamba_variant='mamba2' requires CUDA tensors; use mamba1 for CPU execution")
        return self.mixer(sequence)

...
def main() -> int:
    ...
    train_sets = [build_dataset(scene, samples_per_epoch=args.train_samples, training=True, seed=int(scene.get("seed", 1234))) for scene in configs]
    val_sets = [
        build_dataset(
            scene,
            samples_per_epoch=args.val_samples,
            training=False,
            seed=int(scene.get("seed", 1234)) + 100_000,
            flow_sequences=(train_set.forward, train_set.backward),
        )
        for scene, train_set in zip(configs, train_sets)
    ]

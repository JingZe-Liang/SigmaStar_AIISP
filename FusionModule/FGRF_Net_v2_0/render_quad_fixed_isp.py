...
def main() -> int:
    ...
    try:
        for index in range(frame_count):
            raw_panels = [add_label(tone_map(raw_to_linear(reader(index), metadata, modules, args.highlight_recovery), fixed_scale, pipeline), label) for reader, label in zip(readers, ("2DNR", "3DNR", "FGRF-Net v2 Fusion"))]
            if index < frame_count - 1:
                flow_panel = add_label(flow_to_rgb(load_flow(forward_files[index], (height, width)), flow_maximum), f"Forward RAFT Flow {index:04d}->{index + 1:04d}")
            else:
                flow_panel = add_label(np.zeros((height, width, 3), dtype=np.uint8), "Forward RAFT Flow unavailable")

...
import numpy as np
from Liang.src.isp_utils.CRVD_OpenISP.OpenISP2Transform import OpenISP2transform
import torch
...
def OpenISP_Pipeline(image):
    """
    一个最简单的OpenISP处理流水线（带基本检查）
    """
    # 检查是否是numpy数组
    ...
    # 检查是否是二维的
    if image.ndim != 2:
        raise ValueError("输入必须是2维数组 (H, W)")
...
#         pass
#
#     def fast_isp_core(self, frame_np):
#         # 1. 黑电平扣除与归一化
#         img = (frame_np.astype(np.float32) - 240) / (4095 - 240)
#         img = np.clip(img, 0, 1)
#
#         img[1::2, 0::2] *= 1.9  # R Gain (增加红色，改善僵尸脸)
#         img[0::2, 1::2] *= 1.5  # B Gain (增加蓝色)
#         img = np.clip(img, 0, 1)
#
#
...
#         fuck = isp2transform(x)
#
#         T = 7
#         BT = fuck.shape[0]
#         B = BT // T
#         H, W = fuck.shape[1], fuck.shape[2]
#
#         processed_frames = []
#         for i in range(BT):
#             rgb_frame = self.fast_isp_core(fuck[i])
...
#         plt.imshow(processed_reshaped[0, 0])
#         plt.title("Fast ISP - Zombie Fix Test")
#         plt.axis('off')
#         plt.show()
#
...
class OpenISP2():
    ...
    def forward(self, x: torch.Tensor):
        ...
        T = 7
        # 计算 B
        B = fuck.shape[0] // T  # B = (B*T) // T
        # 遍历处理
        processed_frames = []
        for i in range(fuck.shape[0]):  # 遍历 B*T 次
            frame = fuck[i]
            processed = OpenISP_Pipeline(frame)
            processed_frames.append(processed)

        # 转换为数组并恢复为 (B, T, H, W)
        processed_array = np.array(processed_frames)  # (B*T, H, W)
        processed_reshaped = processed_array.reshape(B, T, *fuck.shape[1:])  # (B, T, H, W)
        return processed_reshaped
    def __call__(self, tensor):
        """让实例可以像函数一样调用"""
        return self.forward(tensor)

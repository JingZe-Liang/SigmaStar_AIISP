...
class ISPModule(nn.Module):
    ...
    参数:
        black_level: 黑电平,默认 240
        white_level: 白电平,默认 4095
        gamma: 伽马值,默认 2.2
    ...
    def pack_gbrg_raw(self, raw: torch.Tensor) -> torch.Tensor:
        ...
        输出: (..., 4, H/2, W/2)
        """
        # 归一化: [black_level, white_level] → [0, 1]
        raw = raw.float()
        raw = torch.clamp(raw - self.black_level, min=0.0) / (
...
def test_isp_module() -> None:
    ...
    # 测试 1: 单帧模式
    print("测试 1: 单帧模式")
    raw_single = torch.randint(
        240, 4095, (2, 1080, 1920), dtype=torch.float32
    )  # (B, H, W)
    ...
    # 测试 2: 序列模式
    print("测试 2: 序列模式")
    raw_seq = torch.randint(240, 4095, (2, 7, 1080, 1920), dtype=torch.float32)  # (B, T, H, W)

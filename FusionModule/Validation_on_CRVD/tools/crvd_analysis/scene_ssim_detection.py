...
from skimage.metrics import structural_similarity as ssim  # 新增导入

# 导入你的 ISP 工具
from Liang.src.isp_utils.CRVD_OpenCV_ISP.OpenCV_ISP import OpenCV_ISP2


def calculate_ssim_np(img1, img2, data_range=4095.0):
    """针对 12-bit RAW 数据计算 SSIM"""
    # SSIM 要求图像维度一致，且为浮点型
...
def verify_all_denoise_datasets():
    ...
    for i, rel_path in enumerate(sampled_rel_paths):
        print(f"\n[{i + 1}/{num_samples}] 正在计算 SSIM 并对比: {rel_path}")

        display_rows = {}
        ssim_table = {}  # 存储 SSIM 数值: {ds_name: [f1, f2...f7]}  （变量名仍可保留为 psnr_table，但语义已变）
        ...
        for ds_name, ds_root in dataset_paths.items():
            ...
            for f_idx in range(1, 8):
                ...
                # 计算与 GT 的 SSIM
                if ds_name == "GT":
                    ssims.append(1.0)  # GT 自身为 1.0
                else:
                    ssims.append(calculate_ssim_np(img, raw_gt_frames[f_idx - 1]))

            if len(raw_frames) == 7:
                raw_tensor = torch.from_numpy(np.stack(raw_frames)).unsqueeze(0)
                with torch.no_grad():
                    display_rows[ds_name] = isp(raw_tensor)[0]
                ssim_table[ds_name] = ssims
            else:
        ...
        for col_idx in range(7):
            # 提取当前列（当前帧）除 GT 外的所有 SSIM
            col_scores = {name: ssim_table[name][col_idx] for name in row_names if
                          name != "GT" and ssim_table.get(name) is not None}
            ...
            for row_idx, ds_name in enumerate(row_names):
                ...
                if frames_rgb is not None:
                    ...
                    # 显示 SSIM 分数（格式化为四位小数）
                    score = ssim_table[ds_name][col_idx]
                    score_str = f"{score:.4f}"  # 去掉 Inf 判断，直接显示数值
        ...
        plt.suptitle(f"SSIM Comparison & Alignment: {rel_path}\n(Red: Best | Orange: 2nd Best)", fontsize=18)
        plt.tight_layout()

...
def _expand_cfa_values(values: Sequence[float], cfa_pattern: str) -> np.ndarray:
    ...
    tiled = np.tile(phase, (HEIGHT // 2, WIDTH // 2))
    return np.asarray(values, dtype=np.float32)[tiled]
...
class WeakFusionDataset(Dataset):
    ...
    def _crop_origin(self, rng: np.random.Generator, motion: np.ndarray) -> tuple[int, int]:
        max_top = HEIGHT - self.patch_size
        max_left = WIDTH - self.patch_size
        if not self.training:
            return max_top // 2, max_left // 2
        if rng.random() < 0.45 and motion.max() > 0:
            candidates = np.column_stack(np.nonzero(motion > 0.5))
            center_y, center_x = candidates[int(rng.integers(0, len(candidates)))]
            top = int(np.clip(center_y - rng.integers(0, self.patch_size), 0, max_top))
            left = int(np.clip(center_x - rng.integers(0, self.patch_size), 0, max_left))
        else:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        full_motion = read_motion_mosaic(sequence, frame_index)
        top, left = self._crop_origin(rng, full_motion)
        bottom, right = top + self.patch_size, left + self.patch_size
        crop = np.s_[top:bottom, left:right]
        stats = self.statistics[sequence.name]
        offset_map = _expand_cfa_values(stats.source_to_dnr_offset, sequence.cfa_pattern)[crop]
        sigma_map = _expand_cfa_values(stats.noise_sigma, sequence.cfa_pattern)[crop]
        ...
        motion = full_motion[crop][None].astype(np.float32)
...
class CleanH5Dataset(Dataset):
    """Original clean-GT dataset retained for Stage 1 pretraining."""

    def __init__(self, root_dir: str | Path, patch_size: int | None = 256, training: bool = True):
        ...
        for path in self.h5_files:
            with h5py.File(path, "r") as handle:
                self.samples.extend((path, index) for index in range(int(handle["clean"].shape[0])))
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        ...
        with h5py.File(path, "r") as handle:
            dnr2 = handle["2dnr"][frame_index].astype(np.float32) / CODE_MAX
            dnr3 = handle["3dnr"][frame_index].astype(np.float32) / CODE_MAX
            clean = handle["clean"][frame_index].astype(np.float32) / CODE_MAX
            noisy = handle["noisy"]
            current = noisy[frame_index, 1].astype(np.float32) / CODE_MAX
            previous = noisy[max(frame_index - 1, 0), 1].astype(np.float32) / CODE_MAX
        if self.patch_size is not None:
            ...
            dnr2, dnr3, clean, current, previous = (item[crop] for item in (dnr2, dnr3, clean, current, previous))

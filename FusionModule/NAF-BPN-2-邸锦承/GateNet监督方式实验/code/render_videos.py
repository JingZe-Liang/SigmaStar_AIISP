...
def parse_args():
    ...
    parser.add_argument("--save-random-frames", type=int, default=0, help="每个序列随机保存的单帧数量")
...
def render_sequence(sequence, result_root: Path, exposure_multiplier: float, random_indices: set[int] | None = None) -> dict:
    ...
    ai_frame_root = result_root / "random_frames" / "ai_fused"
    comparison_frame_root = result_root / "random_frames" / "comparison"
    if random_indices:
        ai_frame_root.mkdir(parents=True, exist_ok=True)
        comparison_frame_root.mkdir(parents=True, exist_ok=True)
    ...
    try:
        for index in range(FRAME_COUNT):
            ...
            comparison = np.concatenate([np.concatenate(small[:2], axis=1), np.concatenate(small[2:], axis=1)], axis=0)
            comparison_writer.write(comparison)
            if random_indices and index in random_indices:
                cv2.imwrite(str(ai_frame_root / f"frame_{index:04d}.png"), panels[3])
                cv2.imwrite(str(comparison_frame_root / f"frame_{index:04d}.png"), comparison)
    finally:
        ai_writer.close()
...
def main() -> None:
    ...
    if args.save_random_frames < 0 or args.save_random_frames > FRAME_COUNT:
        raise ValueError(f"save-random-frames 必须在 0~{FRAME_COUNT} 之间")
    results = []
    for sequence_index, sequence in enumerate(sequences):
        random_indices = None
        if args.save_random_frames:
            random_indices = set(np.random.default_rng(args.random_seed + sequence_index).choice(FRAME_COUNT, size=args.save_random_frames, replace=False).tolist())
        result = render_sequence(sequence, args.output_root / sequence.name, args.exposure_multiplier, random_indices)
        result["random_frame_indices"] = sorted(random_indices) if random_indices else []

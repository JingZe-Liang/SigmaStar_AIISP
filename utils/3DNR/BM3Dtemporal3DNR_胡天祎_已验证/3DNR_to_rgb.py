...
def _stretch_and_gamma(rgb01: np.ndarray, gamma: float = 2.2) -> np.ndarray:
    rgb01 = np.clip(rgb01.astype(np.float32), 0.0, 1.0)
    low = np.percentile(rgb01, 1)
    high = np.percentile(rgb01, 99)
    if high <= low:
        low = float(rgb01.min())
        high = float(rgb01.max())
    if high <= low:
        return np.zeros_like(rgb01, dtype=np.float32)

    out = (rgb01 - low) / (high - low)
    out = np.clip(out, 0.0, 1.0)
    out = np.power(out, 1.0 / gamma)
    return np.clip(out, 0.0, 1.0)


def raw_to_rgb_display(img: np.ndarray, black_level: float, white_level: float, bayer_pattern: str = "RGGB") -> np.ndarray:
    """
    仅用于显示：
    - 若输入是 2D raw Bayer，则先 demosaic 成 RGB
    - 若输入已是 3 通道，则直接做拉伸显示
    不影响 raw 域指标计算。
    """
    if img.ndim == 2:
        pattern = bayer_pattern.upper()
        if pattern not in BAYER_CODE_MAP:
            raise ValueError(f"不支持的 Bayer pattern: {bayer_pattern}")

        raw01 = to_float01(img, black_level, white_level)
        raw16 = np.clip(np.rint(raw01 * 65535.0), 0, 65535).astype(np.uint16)
        rgb16 = cv2.cvtColor(raw16, BAYER_CODE_MAP[pattern])
        rgb01 = rgb16.astype(np.float32) / 65535.0

        # 简单 gray-world 白平衡，仅用于显示
        ch_mean = rgb01.reshape(-1, 3).mean(axis=0)
        mean_all = float(ch_mean.mean())
        gains = mean_all / (ch_mean + 1e-6)
        gains = np.clip(gains, 0.5, 2.5)
        rgb01 = rgb01 * gains[None, None, :]

        return _stretch_and_gamma(rgb01)

    if img.ndim == 3 and img.shape[2] == 1:
        gray01 = to_float01(img[..., 0], black_level, white_level)
        rgb01 = np.stack([gray01, gray01, gray01], axis=-1)
        return _stretch_and_gamma(rgb01)

    if img.ndim == 3 and img.shape[2] == 3:
        # 默认输入是 BGR
        img01 = to_float01(img, black_level, white_level)
        rgb01 = img01[..., ::-1]
        return _stretch_and_gamma(rgb01)

    raise ValueError(f"不支持的显示图像形状: {img.shape}")


def make_error_map_color(img1: np.ndarray, img2: np.ndarray) -> np.ndarray:
    err = np.abs(img1.astype(np.float32) - img2.astype(np.float32))
    if err.ndim == 3:
        err = np.mean(err, axis=2)

    low = np.percentile(err, 1)
    high = np.percentile(err, 99)
    if high <= low:
        low = float(err.min())
        high = float(err.max())
    if high <= low:
        ...
        norm = np.clip((err - low) / (high - low), 0.0, 1.0)

    colored = plt.get_cmap("magma")(norm)[..., :3]
    return colored.astype(np.float32)


def show_comparison_rgb(
    noisy_raw,
    denoised_raw,
    clean_raw=None,
    noisy_psnr=None,
    ...
    noisy_rgb = raw_to_rgb_display(noisy_raw, black_level, white_level, bayer_pattern)
    den_rgb = raw_to_rgb_display(denoised_raw, black_level, white_level, bayer_pattern)
    clean_rgb = None if clean_raw is None else raw_to_rgb_display(clean_raw, black_level, white_level, bayer_pattern)
    ...
    if clean_raw is not None:
        ...
        if noisy_psnr is not None and noisy_ssim is not None:
            t0 += f"\nraw_PSNR={noisy_psnr:.4f}  raw_SSIM={noisy_ssim:.4f}"
        axes[0].set_title(t0)
        axes[0].axis("off")

        axes[1].imshow(den_rgb)
        t1 = "Denoised"
        if den_psnr is not None and den_ssim is not None:
            t1 += f"\nraw_PSNR={den_psnr:.4f}  raw_SSIM={den_ssim:.4f}"
        axes[1].set_title(t1)
        axes[1].axis("off")
        ...
        axes[3].imshow(make_error_map_color(denoised_raw, clean_raw))
        ...
        axes[2].imshow(make_error_map_color(denoised_raw, noisy_raw))
...
def run_bm3d_temporal_3dnr(
    ...
    mean_weight: float = 0.7,
    show_fig: bool = True,
    display_bayer_pattern: str = "RGGB"
):
    ...
    print(f"display_bayer_pattern = {display_bayer_pattern}")
    print("输出文件:", out_path)
    ...
    if clean_raw is not None:
        ...
        print(f"Noisy    -> raw_MSE: {noisy_mse:.12e}, raw_PSNR: {noisy_psnr:.10f}, raw_SSIM: {noisy_ssim:.10f}")
        print(f"Denoised -> raw_MSE: {den_mse:.12e}, raw_PSNR: {den_psnr:.10f}, raw_SSIM: {den_ssim:.10f}")
        print(f"raw_PSNR 变化: {(den_psnr - noisy_psnr):.10f}")
        print(f"raw_SSIM 变化: {(den_ssim - noisy_ssim):.10f}")
    else:
    ...
    if show_fig:
        show_comparison_rgb(
            ...
            bayer_pattern=display_bayer_pattern
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run BM3D-temporal 3DNR and preview the RAW result as RGB.")
    parser.add_argument("target_tiff", help="当前帧路径，例如 scene1/ISO1600/frame5_noisy3.tiff")
    ...
    parser.add_argument("--display-bayer-pattern", default="GBRG", choices=("RGGB", "BGGR", "GBRG", "GRBG"))
...
if __name__ == "__main__":
    ...
    run_bm3d_temporal_3dnr(
        ...
        display_bayer_pattern=args.display_bayer_pattern,
    )
